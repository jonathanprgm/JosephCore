
package net.josephworks.josephcore.item;

import net.minecraftforge.registries.ObjectHolder;

import net.minecraft.item.UseAction;
import net.minecraft.item.ItemStack;
import net.minecraft.item.Item;
import net.minecraft.item.Food;

import net.josephworks.josephcore.itemgroup.JosephCoreItemGroup;
import net.josephworks.josephcore.JosephCoreElements;

@JosephCoreElements.ModElement.Tag
public class BrownieItem extends JosephCoreElements.ModElement {
	@ObjectHolder("josephcore:brownie")
	public static final Item block = null;
	public BrownieItem(JosephCoreElements instance) {
		super(instance, 28);
	}

	@Override
	public void initElements() {
		elements.items.add(() -> new FoodItemCustom());
	}
	public static class FoodItemCustom extends Item {
		public FoodItemCustom() {
			super(new Item.Properties().group(JosephCoreItemGroup.tab).maxStackSize(16)
					.food((new Food.Builder()).hunger(10).saturation(0.5f).build()));
			setRegistryName("brownie");
		}

		@Override
		public int getUseDuration(ItemStack stack) {
			return 20;
		}

		@Override
		public UseAction getUseAction(ItemStack par1ItemStack) {
			return UseAction.EAT;
		}
	}
}
